...
export default function User() {
    ...
    axios
      ...
      .then((res) => {
        if (res.data.status === "success") {
          ...
          setError(res.data.message || "Failed to fetch upcoming events");
        }
      })
      .catch(() => {
  ...
  Welcome {(localStorage.getItem('role') === 'user' && username) ? username : "Guest"} 🎉
</div>
      ...
      <Container className="mt-5 mb-5">
        ...
        {error && (
          <p className="text-center text-danger fs-5 fw-semibold">{error}</p>
        )}

        {!loading && !error && upcomingEvents.length === 0 && (
          <p className="text-center fs-5 text-muted">No upcoming events found.</p>
        )}
        ...
        <Row className="gx-4 gy-5">
          {upcomingEvents.map((event) => {
            ...
            return (
              ...
              >
                <Card
                  className="custom-event-card shadow-sm rounded-4 w-100 border-0"
                >
                  <Card.Body className="d-flex flex-column">
                    ...
                    <div className="mb-3">
                      ...
                      <p className="mb-2 fs-6">{eventDate.toLocaleDateString(undefined, {
                        year: "numeric",
                        month: "short",
                        day: "numeric",
                      })}</p>
                    ...
                    <div className="mb-3 flex-grow-1">
                      <p className="fw-semibold text-muted" style={{ fontSize: "0.85rem" }}>
                        📝 Description
                      </p>
                      <p className="fs-6 text-truncate" style={{ maxHeight: "5rem", overflow: "hidden" }}>
